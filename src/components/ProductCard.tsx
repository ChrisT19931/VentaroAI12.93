    ...
    <div className={`glass-panel rounded-lg overflow-hidden transition-all duration-200 hover:shadow-xl ${className}`}>
      <Link href={productUrl} className="block relative">
        <div className="relative h-48 w-full overflow-hidden bg-gradient-to-br from-gray-900 to-black">
          ...
          {productType && (
            <span className={`absolute top-2 left-2 text-xs font-medium px-2 py-1 rounded-full ${productType === 'digital' ? 'bg-emerald-500/80 text-white' : 'bg-purple-500/80 text-white'}`}>
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between items-center">
            <span className="text-lg font-bold text-emerald-400">{formatPrice(price)}</span>
          </div>
        </div>
      </Link>
          ...
          <button
            ...
            className={`w-full py-2 px-4 rounded-md transition-all duration-200 ${alreadyInCart
              ...
              : isAdding
                ...
                : 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 hover:shadow-emerald-700/30 hover:scale-[1.02]'}`}
          >
            {alreadyInCart
              ? 'Added to cart'
              : isAdding
